...
class App(title: String, settingsText: String?) : JFrame(title) {
    ...
    private val downloadButton = JButton("Download the current chain").apply {
        addActionListener {
    ...
    private val client by lazy {
        Client(
                ...
                onReceiveBlockFromPeer = { block ->
                    clientOnReceiveBlockFromPeer(block)
                },
                onLog = { message ->
                    appendLogText(message)
                }
    ...
    private fun clientOnReceiveBlockFromPeer(block: Block) {
                        ...
                        JSplitPane(
                                ...
                                JSplitPane(
                                        ...
                                        JSplitPane(
                                                ...
                                                poolPanel,
                                                chainPanel
                                        ).apply { dividerLocation = 120 },
                                        blockPanel
                                ).apply { dividerLocation = 320 },
                                transactionPanel
                        ).apply { dividerLocation = 400 }
                )
    ...
    private fun onClickDownloadButton() {
        client.requestChainToOneOfPeers()
    }
    ...
    private fun setTransactionViewer(transaction: Transaction) {
        val model = transaction.getModel(chain)

        if (model.faces.isNotEmpty()) {
            transactionViewer.setMesh(Mesh.fromModel(transaction.getModel(chain)))
        }
    }
